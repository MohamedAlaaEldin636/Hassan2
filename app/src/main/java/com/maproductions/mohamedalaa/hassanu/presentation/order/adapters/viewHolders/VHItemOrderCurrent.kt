...
class VHItemOrderCurrent(parent: ViewGroup, private val gson: Gson) : RecyclerView.ViewHolder(
    ...
    init {
        ...
        binding.providerMSGMaterialCardView.setOnClickListener {
            ...
            val otherName = binding.serviceTypeTextView.tag as? String ?: return@setOnClickListener
    ...
    fun bind(item: ResponseOrder) {
        ...
        binding.providerCallMaterialCardView.tag = item.provider?.phone
        binding.providerMSGMaterialCardView.tag = item.provider?.id
        binding.providerClickableView.tag = item.provider?.toJson(gson)
        binding.providerNameTextView.tag = item.provider?.imageUrl
        binding.serviceTypeTextView.tag = item.provider?.name

        binding.imageImageView.setUrlViaGlideOrPlaceholder(item.provider?.imageUrl)
        binding.providerNameTextView.text = item.provider?.name.orEmpty()
        binding.providerRatingBar.setProgressBA(
            (item.provider?.averageRate.orZero() * 20f).roundToInt()
        )
        binding.providerRateTextView.text = "(${item.provider?.averageRate.orZero().roundHalfUpToIntOrFloat(1)})"
