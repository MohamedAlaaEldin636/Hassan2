...
import com.maproductions.mohamedalaa.hassanu.presentation.service.adapters.viewHolder.VHServiceInSelection
...
class RVServiceInSelection(private val gson: Gson) : PagingDataAdapter<ServiceInCategory, VHServiceInSelection>(COMPARATOR) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VHServiceInSelection {
        return VHServiceInSelection(parent, gson) { position, service, increment ->
            incrementOrDecrementThenGetNewCount(position, service, increment)
        }
    }

    override fun onBindViewHolder(holder: VHServiceInSelection, position: Int) {
